/* ═══════════════════════════════════════════════
   ...
   Navigation · Match setup · Viewer · App init
   ═══════════════════════════════════════════════ */
...
// ── All page ids ──────────────────────────────────
var ALL_PAGES = ['pg-home','pg-setup','pg-score','pg-viewer','pg-result','pg-saved','pg-lb'];
...
function goPage(id, cb) {
  ...
  var flex = ['pg-home','pg-score','pg-viewer'];
  el.style.display = flex.indexOf(id) >= 0 ? 'flex' : 'block';
...
function toast(msg, dur) {
  ...
  clearTimeout(_tT);
  _tT = setTimeout(function() { el.style.opacity = '0'; }, dur || 2600);
}
...
function goSetup() {
  if (!G.pA.length) {
    for (var i = 1; i <= 11; i++) {
      G.pA.push(mkP('Player A' + i));
      G.pB.push(mkP('Player B' + i));
    }
  }
  renderPls('A'); renderPls('B');
  goPage('pg-setup');
}

function renderPls(t) {
  var arr = t === 'A' ? G.pA : G.pB;
  var wrap = E(t === 'A' ? 'plA' : 'plB');
  wrap.innerHTML = '';
  arr.forEach(function(p, i) {
    var d = document.createElement('div');
    d.className = 'plrow';
    d.innerHTML = '<div class="plnum">' + (i+1) + '</div>'
      + '<input value="' + esc(p.name) + '" placeholder="Player ' + (i+1) + '" oninput="namePl(\'' + t + '\',' + i + ',this.value)">'
      + (arr.length > 1 ? '<button class="rmbtn" onclick="rmPl(\'' + t + '\',' + i + ')">×</button>' : '');
    wrap.appendChild(d);
  });
  tx(t === 'A' ? 'cntA' : 'cntB', arr.length + ' player' + (arr.length !== 1 ? 's' : ''));
}
...
function startMatch() {
  ...
  G.penBonus = {A:0,B:0}; G.penMinus = {A:0,B:0};
  G.hist = []; G.inn = {};
  G.done = false; G.headline = ''; G.winTeam = null; G.motm = null;
  G.joinCode = null; G.firestoreDocId = null;
  G.innings = 1; G.cur = 1;
...
function openInnModal(n) {
  ...
  tx('op-title', tname(inn.bat) + ' — Openers (Inn ' + n + ')');
  showOv('ov-openers');
}
...
function confirmOpeners() {
  ...
  inn.striker    = bp[i1];
  inn.nonStriker = bp.length > 1 ? bp[i2] : null;
  ...
  inn.batterIdx = 2;
  inn.bowler = bwp[parseInt(E('op-bwl').value)];
...
function newMatch() {
  ...
  G = { pA:[], pB:[], hist:[], inn:{}, penBonus:{A:0,B:0}, penMinus:{A:0,B:0},
        innings:1, cur:1, tA:'', tB:'', overs:20, brkMins:10,
        joinCode:null, firestoreDocId:null, done:false, pg:'pg-home',
...
function joinViewer() {
  ...
  joinMatchByCode(raw, function(data, docId) {
    ...
    ih('v-hostbadge', FB.isHost
      ? '<span class="host-badge">HOST</span>'
      : '<span class="viewer-badge">👁 VIEWER</span>');
    goPage('pg-viewer');
    if (data.gameState) {
      try { var vG = JSON.parse(data.gameState); renderVLive(vG); renderVSC(vG); } catch(e) {}
    }
...
function stopViewer() { stopRealtimeListener(); goPage('pg-home'); }

function viewerRefresh() {
  if (!FB.matchDocRef) { toast('Not connected'); return; }
  _fsGet('matches/' + FB.matchDocRef, function(doc) {
    var data = _fromFSDoc(doc);
    if (!data) { toast('Cannot reach server'); return; }
    if (data.gameState) {
      try { var vG = JSON.parse(data.gameState); renderVLive(vG); renderVSC(vG); toast('Refreshed ✓'); } catch(e) {}
    }
  });
}

function vTab(t) {
  document.querySelectorAll('#vtabs .tab').forEach(function(el,i){ el.classList.toggle('on', i===t); });
  ['vt-live','vt-sc','vt-lb'].forEach(function(id,i){ sd(id, i===t?'block':'none'); });
}
